...
public class CameraActivity extends Activity
    ...
    public static final boolean YIXI_LOG_ON = true;
    //add by lilei end
    ...
    private class MyOrientationEventListener
        ...
        public void onOrientationChanged(int orientation) {
            ...
            int orientationCompensation =
                    (mOrientation + CameraUtil.getDisplayRotation(CameraActivity.this)) % 360;
            if (mOrientationCompensation != orientationCompensation) {
            	if(YIXI_LOG_ON){
            		Log.i("lilei","onOrientationChanged orientation:"+orientation+" orientationCompensation:"+orientationCompensation
                        +" mOrientationCompensation:"+mOrientationCompensation);
            	}
                mOrientationCompensation = orientationCompensation;
                //setOrientationIndicator(mOrientationCompensation, true);
                mCurrentModule.onOrientationChanged(mOrientationCompensation);
            }
            //modify by lilei end
        }
    }
    ...
    private void updateActionBarMenu(int dataID) {
        ...
        //no google map now  added by lilei begin
        String strPackageName = "com.google.android.apps.maps";
        boolean isShowOnMap = false;
        final PackageManager packageManager = this.getPackageManager();
        final Intent mainIntent = new Intent(Intent.ACTION_MAIN, null);
        List<ResolveInfo> apps = packageManager.queryIntentActivities(mainIntent, 0);
        for(int i=0; i< apps.size();i++){
            ResolveInfo app = apps.get(i);
            if(app.activityInfo.packageName.equals(strPackageName)){
                isShowOnMap = true;
                break;
            }
        }
        ...
        setMenuItemVisible(mActionBarMenu, R.id.action_show_on_map,
                itemHasLocation && (supported & SUPPORT_SHOW_ON_MAP) != 0 && isShowOnMap);
    }
    ...
    //add by lilei begin
    public LocalDataAdapter getFixedFirstDataAdapter(){
        return mWrappedDataAdapter;
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        if (ev.getActionMasked() == MotionEvent.ACTION_DOWN) {
            ...
            //dismiss clolor effec pop add by lilei begin
            if(mCurrentModule instanceof PhotoModule){
                float x = ev.getX();
                float y = ev.getY();
                Log.i("lilei","111 dispatchTouchEvent result:"+result);
                //Log.i("lilei", "dispatchTouchEvent  getColorEffectSetListener:"+((PhotoModule)mCurrentModule).getColorEffectSetListener());
//                YixiColorEffectButton mColorEffectButton = (YixiColorEffectButton)findViewById(R.id.btn_color_effect);
//                YixiColorEffectSettingPopup mColorEffectPop = (YixiColorEffectSettingPopup)findViewById(R.id.color_effect_pop);
//                
////                Log.i("lilei","22 dispatchTouchEvent btn:"+mColorEffectButton+" pop:"+mColorEffectPop+" visable:"
////                        +(mColorEffectPop == null ? "null":mColorEffectPop.getVisibility()));
//                if(mColorEffectPop != null && !CameraUtil.pointInView(x,y,mColorEffectButton) && !CameraUtil.pointInView(x,y,mColorEffectPop)){
//                    Log.i("lilei","333");
//                    ((PhotoModule)mCurrentModule).getColorEffectSetListener().dismissPop();
//                }
//                if(mColorEffectPop != null && CameraUtil.pointInView(x,y,mColorEffectPop)){
////                    Log.i("lilei","444 dispatchTouchEvent");
////                    return false;
//                }
                //add by liei end
            }
    ...
    public void onResume() {
        ...
        if (Settings.System.getInt(getContentResolver(),
        	...
        	Log.i("lilei", "onResume) 111 SCREEN_ORIENTATION_PORTRAIT");
            setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
            mAutoRotateScreen = false;
        } else {
        	Log.i("lilei", "onResume) 222");
            setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
    ...
    protected void updateStorageHint(long storageSpace) {
        ...
        if (storageSpace == Storage.UNAVAILABLE) {
            ...
            if(Storage.isSaveSDCard()){  //add for AP-222 by lilei
        ...
        if (message != null) {
            ...
            mStorageHint.show();
            //add for AP-394 by lilei begin
            if(mCurrentModule instanceof PhotoModule){
                PhotoModule mPhotoModule = (PhotoModule)mCurrentModule;
                mPhotoModule.setShutterButtonFocus(false);
            }
            //add for AP-394 by lilei end
        } else if (mStorageHint != null) {
            mStorageHint.cancel();
            mStorageHint = null;
        }
    }
    public void scanCacheDir(){  //add for AP-111 by lilei
        String actionScanFile = "android.intent.action.MEDIA_SCANNER_SCAN_FILE";
        Uri uri = Uri.parse("file:///storage/emulated/0/Android/data"); //cache dir
        Intent i = new Intent(actionScanFile,uri);
        Log.i(TAG,"~~~scanCacheDir() dir  intent:"+i+"uri:"+uri);
        sendBroadcast(i);
    }
    ...
    public void onModuleSelected(int moduleIndex) {
    	Log.i("lilei", "onModuleSelected() mCurrentModuleIndex:"+mCurrentModuleIndex+" moduleIndex:"+moduleIndex);
        if (mCurrentModuleIndex == moduleIndex) {
            return;
        }

        CameraHolder.instance().keep();
        Log.i("lilei", "closeModule() mCurrentModule:"+mCurrentModule);
        closeModule(mCurrentModule);
        setModuleFromIndex(moduleIndex);
        Log.i("lilei", "openModule() mCurrentModule:"+mCurrentModule);
    ...
    private void openModule(CameraModule module) {
        ...
        if(module instanceof VideoModule){
        	final CameraModule mModule = module;
        	mMainHandler.postDelayed(new Runnable() {
				@Override
				public void run() {
					// TODO Auto-generated method stub
		        	((VideoModule) mModule).onShutterButtonClick();
				}
			}, 50);
